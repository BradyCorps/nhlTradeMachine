...
- [x] Freeze the resolved-contract calibration protocol and audit leakage/sample coverage (September 9, 2026): `scripts/backtest/nav-calibration-cohort.ts` produces all requested resolved-versus-unresolved distributions, keeps 2023–24 train, 2024–25 validation and 2025–26 sealed holdout forward-only, and confirms all F/D/G sample floors pass (390/513/651 joined rows). The proposed Phase 5 shadow target is annual cap share from deal-excluded pre-signing positional signals. See `docs/analytics/NAV01_CONTRACT_CALIBRATION_PROTOCOL.md`; no public values or migration begin here.
- [ ] Remedy the failed resolved-cohort representativeness gate before a full-cohort shadow fit: resolve at least 280 additional ELC rows, with at least 59 aged 18–21, and confirm their dated pre-signing performance coverage. This is a minimum selected subset, not a requirement to resolve all 1,918 records. Treat blank signing status as a missing-data stratum with lower coverage and wider uncertainty, never an identity-imputation request.
