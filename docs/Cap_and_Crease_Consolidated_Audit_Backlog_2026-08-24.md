...
**Progress — August 31, 2026 (increment 2 of `L`; ticket remains `[ ]`).** Researched the actual shape of the remaining scope before writing code, rather than assuming what "not yet built" from increment 1 implied. Findings, and what they changed about the plan:

- Contract status (`SIGNED`/`RFA`/`UFA`) is genuinely already centralized — `deriveContractStatus` (`roster-assembly.ts`) is the one function every one of the six surfaces (Players list, player dossier, Teams, Trade Machine, Armchair GM, Fantasy) reads through via the shared cached `assembleCanonicalRoster()`/`getCachedRoster()` pipeline. "Migrate six independent derivations to one" was the right framing for the ticket's opening bug, but is largely already true for the fields that exist today; the real gap is narrower than it first read.
- Found a real, live gap instead: two **independent** signals both claim to answer "is this player unsigned" — `contractStatus` and roster location (`teamId === FA_POOL`, `fa-pool.ts`) — and nothing checks they agree. A stale write in the offseason state machine (`free-agency.ts` moving a player to `FA_POOL` without also clearing `contractStatus`, or the reverse) would silently produce a player reading as both "on the open market" and "signed," undetected. Added `freeAgentPoolConsistencyInvariant` to `release-gates.ts` (DATA-06's existing home for exactly this class of check) — flags a player in `FA_POOL` whose `contractStatus` still reads `SIGNED`. Deliberately does NOT flag a pending RFA/UFA still on his current roster (mid-season, rights not yet reached market) — that is the normal state, not a contradiction, the same direction-matters lesson `contractStatusAgeInvariant`'s own history in this file already states. Not yet run against a real production export — this environment has no live DB access (see Environment notes); dogfood it the way `contractStatusAgeInvariant` was, next time a real dump is available.
- Confirmed the offseason population-conservation invariant (`roster + retained rights + RFA + UFA + signed elsewhere + retired = previous population + drafted players`) is **already implemented and tested** — `auditOffseasonPlayerStates` in `app/lib/offseason-ledger.ts`, shipped under a prior sim-engine audit ticket. It is pure and generic (takes plain `Asset[]`), but every current caller is Armchair GM's in-memory simulation state (`useOffseasonFlow.ts` and its siblings) — it has never been run against the real, canonical DB-backed roster population, because there is no stored historical roster snapshot to diff against (this app doesn't persist roster-over-time; it only reads the live current state). Applying it to real data is blocked on a genuine product/storage decision (start snapshotting rosters at season boundaries?), not a coding gap — documenting rather than fabricating a fixture with no real "previous population" behind it.
- Confirmed contract effective-date overlap checking (the acceptance line's "no active contract falls outside its effective window or overlaps another active contract") is **not buildable today at all**: there is no contract start-date column anywhere (`players` table has `expiryYear`/`yearsRemaining` only — an end anchor, by `contract-term.ts`'s own explicit design), and the schema stores one contract row per player, not a contract history, so there is no second record to overlap against even if a start date existed. This needs a schema decision (add a start date? model contract history as its own table?) before it can be scoped as an increment, not a checkable oversight in existing code.
- Looked at wiring the unused `players.injuryStatus`/`isLtir` DB columns into `Asset` (the "injured/reserve status" canonical field) and deliberately did NOT — no code anywhere writes either column (no injury-report ingestion exists), and `assembleCanonicalRoster` is the largest, most delicate, highest-blast-radius function in the codebase. Wiring a field through it that would read `null`/`false` for every player today, with zero source and zero consumer, is pure regression risk for zero current payoff. Ready to wire once a real data source exists — not before.
- Evidence: full suite **2,369/2,369** (3 new focused tests for the new gate), TypeScript and changed-file lint clean, production build passes (**30/30** static pages).
- Remaining `DATA-01` scope, now more precisely stated: the unified ledger object itself (worth building once the researched gaps below have real data behind them, so it isn't mostly null fields); wiring injury/reserve status once a real feed exists; a schema decision on contract start-date/history before overlap-checking is even buildable; and applying the already-built population-conservation invariant to real roster snapshots once this app stores any.
...
**Progress — August 31, 2026 (increment 2 of `L`; ticket remains `[ ]`).** Researched what "wire Fantasy onto X-NAV" should actually mean before touching code — Fantasy's own VOR/FP82 model (`app/lib/fantasy-board.ts`, league-configurable fantasy scoring vs. a replacement-level baseline) is a real, tested, from-scratch statistical model answering a different question than X-NAV (a cap-surplus/trade-value model — cap hit, contract term, defensive value, none of which a fantasy-points owner cares about). The acceptance line names the **snapshot ID**, not the ranking — "UI components may format this object but must not independently recalculate it" — so this increment adds X-NAV's provenance envelope to Fantasy as a labelled, secondary cross-reference, and leaves VOR/FP82/tiers exactly as the ranking signal:

- Fantasy already fetches `/api/league/players` — and that response already carries `navMap` (`{ ...value, provenance }` where `value` is `getCachedRoster()`'s cached payload, which has included `navMap: buildLeagueNavMap(...)` since DATA-06's precompute work). Fantasy's page code simply never read the field it was already receiving; wiring it in cost zero new network calls, zero new server-side compute, and did not touch `fantasy-board.ts`'s tested ranking math at all.
- Added a small `XNavStrip` display block to the existing expanded-row Ledger-outlook panel (both the mobile card and desktop table variants) showing X-NAV total, coverage, uncertainty range (when present), model version, `asOf`, and a truncated `snapshotId` — clearly labelled via `HelpPopover` as a different question from fantasy points and never used to rank or filter the board.
- Found and fixed a real latent type gap while wiring this: `trade-types.ts`'s `XNAVResult` was a **hand-mirrored copy** of `xnav-engine.ts`'s (the file's own comment already flagged this as "worth collapsing into one definition") and was missing `.snapshot` entirely — so the 25 files across the app that import `XNAVResult` from `trade-types.ts` (nearly every UI surface) could not see the DATA-02 envelope at all without an unsafe cast. `app/lib/league-nav.ts` carried exactly that cast (`as XNAVResult`). Added `snapshot?: ValuationSnapshot` to the mirrored interface and removed the now-unnecessary cast — every one of those 25 consumers can now read `.snapshot` with no workaround, which is what "UI components may format this object" actually requires structurally, not just at the two routes that happened to return it verbatim.
- Not verified live in a browser: this sandbox's fallback/degraded local dataset produces a 0-skater draft board (the same "Player analytics source is unavailable" condition noted elsewhere in this session), so there were no rows to expand and click through. Verified instead via `tsc` (clean), a direct `curl` of `/api/league/players` confirming `navMap[id].snapshot` is present with all expected fields, and a new regression test (`league-nav.test.ts`) asserting the snapshot survives the type fix with no cast.
- Evidence: full suite **2,370/2,370**, TypeScript and changed-file lint clean, production build passes (**30/30** static pages).
- Remaining `DATA-02` scope: `snapshotId`/`modelVersion`/`asOf`/`uncertainty`/`coverage` are still not shown on Players/Teams/Trade/Armchair (Fantasy is now the one surface that does); no persisted snapshot manifest or cross-day audit trail (`DATA-06`); and the live McDavid/Thompson `V-04` route re-comparison still needs a real environment with DB/NHL egress.
...
**Progress — August 27, 2026 (increment 1 of `L`; ticket remains `[ ]`).** Fixed the one named, verifiable canary and closed several of the concrete acceptance bullets; did not build the unified contention object, the reserve-list/prospect population split, or a live 32-team verification pass:

- **Ian Cole canary, verified live and fixed.** He signed a real 1-year/$4.0M UFA deal with Chicago on July 1, 2026 (nhl.com/blackhawks, puckpedia) — the committed seed still carried a stale $2.8M with no expiry class, so he could never appear in a 2027 expiry count. Corrected via the same `SEED_CORRECTIONS` mechanism used for `DATA-01` (`contracts.bundled.json` + `scripts/build-league-seed.ts`, regenerated `league-seed.json`), setting `expiryStatus: "UFA"`, `expiryYear: 2027` directly rather than through the 2026 FA-class list (he is not expiring this offseason).
- **Added `app/lib/expiry-ledger.ts`** (`expiryCountsByYear` / `teamExpiryCountsByYear`): groups every player with a known free-agency class by the calendar year their rights actually reach the market, from the raw `expiryStatus`/`expiryYear` facts rather than `contractStatus` (which collapses every not-yet-expired contract to `SIGNED` and made every future offseason's class invisible until the year it happened — the literal "unexplained zero" the acceptance bullet names). A live test against the committed seed confirms Ian Cole now appears under 2027.
- **Wired the existing `rosterLegality` counter into `/api/league`'s per-team payload** as `lineupAccounting` — the same pure 12F/6D/2G counter the Armchair GM simulation gate already trusted, now exposed with named, machine-readable vacancies for every club in the shared league response rather than only gating simulation.
- **Verified the phase/window separation is already structurally fixed** (`app/lib/team-window.ts`, `app/armchair-gm/page.tsx`): Armchair GM writes `rosterWindow`, never `phase`; nothing overwrites the standings tier anymore. The remaining gap was that this distinction was undocumented anywhere a user could read it.
- **Added a "Team Model" section to `/methodology`** publishing, in one place: the phase-vs-window distinction and why both are real; the $95.5M curated cap-space reference ceiling and ceiling-delta rebasing rule (the previously-shipped $8.5M-discrepancy fix); the 12/6/2 lineup-legality minimums; the F/D/G-NAV population (signed active roster only, a client-side positional sum today — not yet an independent model, consistent with `V-06`) and that it is not yet independently calibrated; and the expiry-ledger's by-year rather than by-bucket accounting.
- Evidence: 15 new focused tests (`expiry-ledger`, `data03-team-model-methodology`) plus existing `league-seed`/`roster-legality` coverage exercised unchanged, full suite **2,262/2,262**, TypeScript clean, changed-file lint clean, production build passes (**30/30** static pages).
- Not yet done (remaining `DATA-03` scope): no single "contention object" exists — phase, window, cap flexibility, future asset base, and Cup outlook are still separate fields/functions rather than one carried, provenance-stamped record; active roster / reserve list / prospects are not yet stored as three separate populations anywhere; `season`/`asOf`/`source`/`coverage`/`modelVersion` are not attached to team-level aggregations the way `DATA-02` attached them to player valuations; and Rangers/Chicago/Toronto were not re-verified against a live build in this session (no DB/NHL egress here) — the phase/window fix was confirmed by reading the code path, not by rendering the three named teams.

**Progress — August 31, 2026 (increment 2 of `L`; ticket remains `[ ]`).** Researched the actual shape of "phase, window, cap flexibility, future asset base, and Cup outlook" before building anything: "Cup outlook" is not a separate existing concept anywhere in the codebase — it is the same present/future contention quadrant `computeContention` already produces, just under a different name in the ticket text. `computeContention` itself is called independently from four separate places (Armchair GM's page, its sim dispatch hook, its analysis tabs, and the Teams page) with no shared carried record between them — the actual duplication DATA-03 is about. Built the team-level equivalent of DATA-02's `ValuationSnapshot`:

- Added `app/lib/team-contention-snapshot.ts`: a `TeamContentionSnapshot` (`snapshotId`, `teamId`, `asOf`, `modelVersion`, `phase`, `rosterWindow`, `window`, `contention`, `capSpace`, `capBreakdown`, `coverage`) built from already-computed inputs — it does not run `computeContention`/`teamWindow`/cap resolution itself, mirroring `buildValuationSnapshot`'s exact relationship to `calcNAV`'s output. `snapshotId` is content-addressed (same `stableStringify`+SHA-256+day+modelVersion approach as DATA-02; exported `stableStringify` from `valuation-snapshot.ts` rather than a second copy). `window` is computed by calling the existing `teamWindow()` reconciler rather than re-deriving it, so the snapshot's window can never disagree with what every other surface already shows — the "unexplained contradictory phase/window labels" acceptance line is satisfied by construction. `coverage` is `"full" | "partial" | "phase-only"` from whether a live contention read and a real cap breakdown both exist, mirroring `ValuationCoverage`'s three-way shape.
- **No single team-aggregation choke point exists** the way `calculateAssetNAV` is one for players — `/api/league` and `/api/league/teams` each independently duplicate their own `loadTeams()`/team-object-construction logic (confirmed nearly line-for-line duplicated: `derivePhase`, standings-fetch/sort, the `TEAMS_DB.map()` pattern). Rather than attempt that larger route-consolidation project in this increment, wired the snapshot builder into the one place where phase, a live roster read, contention, and cap space already converge today: `app/teams/page.tsx`'s `teamProfiles` computation. Also called the existing `deriveTeamPhase(roster, navMap)` from there for the first time — previously only Armchair GM computed a live roster window; Teams now gets the same real signal rather than only ever showing the standings tier.
- **The one visible chip** on each team card now shows `contentionSnapshot.window` (the reconciled label) instead of the raw standings `phase`; when a live roster read genuinely diverges from the standings tier, an explicit, accessible `HelpPopover` note explains why rather than leaving two different labels for two different surfaces to show without explanation.
- Verified live in a browser (32 real teams, real standings/cap data): all cards render their reconciled chip with zero console/page errors; expanding a card causes no crash. This sandbox's roster-analytics data is too sparse for any team to clear `deriveTeamPhase`'s qualified-player floor, so no live divergence badge was observed — the fallback-to-phase path (the common case on real data too, since a full live window needs real analytics) is exactly what rendered, and is exercised directly by 12 new focused tests.
- Evidence: full suite **2,382/2,382**, TypeScript and changed-file lint clean, production build passes (**30/30** static pages).
- Remaining `DATA-03` scope: active roster/reserve list/prospects still are not stored as three separate populations; the `/api/league` vs `/api/league/teams` route duplication remains (a separate, larger consolidation); the snapshot is not yet attached server-side or persisted (`DATA-06`); and Armchair GM's three other `computeContention` call sites still recompute independently rather than reading a carried snapshot.
...
**Progress — August 27, 2026 (increment 1 of `L`; ticket remains `[ ]`).** Two of the five acceptance bullets were already true and are recorded here rather than reworked; the other three got real fixes or an honest partial:

- **Rounds 1–7, fixed.** `buildDraftPickInventory` (the Trade Machine/league inventory), `/api/admin/draft-picks` (the seed the admin editor reads), and the admin pick editor's round filter had each separately hand-copied `[1, 2, 3, 4, 5]` — every club's 6th- and 7th-round picks were silently absent from all three surfaces at once, the exact defect named in the acceptance bullet. Added one canonical `ALL_DRAFT_ROUNDS = [1,2,3,4,5,6,7]` (`app/lib/draft-picks.ts`) and pointed all three call sites at it.
- **Traded-pick persistence and the "2027 first can't come back in year two" case were already true**, verified by reading `dropSpentDraftPicks` and its existing test `"removes a traded pick from its NEW owner too"` — a traded pick is removed from both the original owner's pool and the acquiring team's pool once its draft year completes, and the ownership override (`draft_pick_overrides`, keyed by original-team+year+round) is what makes `buildDraftPickInventory` assign it to the current owner in the first place. No change needed.
- **The Viggo Björck/Bjorck duplicate-selection case was already true**, verified by reading `draft-reconcile.ts` and its existing test suite — it exists specifically for this scenario: an accent-stripped seeded prospect and the same player's drafted rookie row are merged by normalized name (backfilling draft context onto the existing row) rather than both surviving as two selections. No change needed.
- **Pick value: partial.** `calcPickNAV`'s rounds 4 through 7 shared one flat constant (`2`) regardless of round or standing — a lottery team's 4th priced identically to a contender's 7th, which would have been invisible before this pass (rounds 6-7 had no tradable asset to expose it) and became a real inconsistency the moment rounds 6-7 went live. Extended the existing round-3 two-tier standing shape down through round 7 (monotonically decreasing by round, standing-sensitive) so the newly-tradable late rounds are at least ordered and distinct. This is explicitly **not** the calibrated-by-year/round/position distribution the bullet asks for — per the project's model-validation rule, a real pick-value curve needs draft-slot-to-outcome or trade-market evidence and its own backtest, and no such data was reachable in this session (no NHL egress, no historical trade-value dataset in the repo). The mechanical extension is documented in code as an interim shape, not a validated model.
- Evidence: 8 new/updated focused tests (`ALL_DRAFT_ROUNDS`, seven-round inventory generation, round 3-7 monotonicity and floor/ceiling bounds for `calcPickNAV`) plus one source-text canary corrected from asserting the rounds-1-5 bug to asserting the fix. Full suite **2,267/2,267**, TypeScript clean, changed-file lint clean, production build passes (**30/30** static pages).
- Not yet done (remaining `DATA-04` scope): no unified pick object carries conveyance alternatives (e.g. "1st, converts to two 2nds if X") or full transaction history — only current ownership and a single protection/condition string exist; and pick value has no calibrated, evidence-backed distribution, only the corrected placeholder shape above.
...
**Progress — August 27, 2026 (increment 1 of `L`; ticket remains `[ ]`).** One acceptance bullet was a real, live gap and got fixed; two were already true and are recorded rather than reworked; two remain open:

- **The four-retained-contract stress test, fixed.** The three-slot CBA rule (`MAX_RETENTION_SLOTS = 3`) existed only inside `cup-run.ts`, so only Armchair GM's Cup Run simulation ever enforced it — Trade Machine's `/api/evaluate` checked the 50%-per-contract rule but had no count-based slot check at all, so one proposal could retain on four separate players with nothing to stop it. Extracted the rule into `app/lib/retention-ledger.ts` (re-exported from `cup-run.ts` so every existing caller is unaffected) and added a same-trade slot count (`retainedSlotsInTrade`) checked independently for both sides in `runGmLogic`, producing a `HARD`/`RETAIN_ABUSE` veto — verified end-to-end through the real route: four retained contracts blocks, three does not.
- **0–50% retention consistency was already correct.** The engine (`xnav-engine.ts`) already applies `capHit × (1 − retainedPct)` consistently to both cap surplus and value; the trade UI (`AssetCard.tsx`) already clamps the retention control to 0–50 in 5% steps; the existing 50%-hard-veto flag already blocks anything above it. No change needed.
- **Cap-total reconciliation: real duplication found and reduced, not fully unified.** `CapProjection.tsx` (unused/unrendered anywhere in the app — verified by repo-wide search) recomputed "cap used" by summing the roster's raw contract rows from scratch — the exact "naive sum" `team-cap-space.ts` documents as overstating used cap because it can't see LTIR relief, buried contracts, or bonus overages — instead of starting from the same curated `capSpace` baseline Teams and the live trade summary use; fixed it to use the shared `applyCapDelta` from a curated baseline, and added the missing "Cap Before Trade" line so before and after are both shown and reconcile by construction. The **live** Trade Machine summary (`QuickTradeMachine.tsx`) already did this correctly (curated baseline + delta), but had its own fourth independent copy of the effective-cap-hit formula (after `xnav-engine.ts`, `cap-delta.ts`, and `CapProjection.tsx`'s now-removed copy) — deduped onto the one shared `effectiveCapHit` in `cap-delta.ts`. This closes duplication that was a latent "must agree" risk, not a currently-wrong live number.
- **The UFA-vs-retained-RFA cap-priority case was already correct.** `free-agency.ts`'s `resolveLeagueOffseason` books every RFA's re-signed cap hit unconditionally, for every team, before the separate market-signing pass runs against the resulting cap map — a team cannot sign an external UFA "at its own RFA's expense" by construction (the code's own comment cites this as a previously-fixed bug). No change needed.
- Evidence: 11 new/updated focused tests (`retention-ledger`, two new `evaluate-route` integration tests hitting the real route, updated source-text canaries), full suite **2,278/2,278**, TypeScript clean, changed-file lint clean, production build passes (**30/30** static pages).
- Not yet done (remaining `DATA-05` scope): NMC/NTC/M-NTC clauses are still plain booleans defaulting to "does not have," with no `unknown/unverified` state distinguishing "verified absent" from "never checked" — a real gap spanning the DB schema and roughly 26 consuming files, genuinely out of reach for this session (a schema migration + full read-path audit, not a bounded fix); and there is still no single persisted "reconciliation object" carrying retained-salary dollar amount / original club / acquiring club / occupied-slot detail together — only the pieces (`RetentionEntry`, cap deltas, flags) exist separately.
...
**Progress — August 27, 2026 (increment 1 of `M`; ticket remains `[ ]`).** Built the gate/manifest infrastructure this ticket names and wired the cache-invalidation acceptance line into the two hot, dynamically-keyed team caches; did not wire all eight domains to live ingest signals or touch the fully-static payload cache keys:

- **`app/lib/release-gates.ts`** — seven named, independently-reportable `GateResult` checks, one per bullet the ticket lists, each a pure function reusing an existing DATA-01–05 primitive rather than re-deriving it: `exactNameAliasInvariance` (wraps `canonicalNameSlug`/`makePlayerId`), `crossSurfaceValueReconciliation` (checks DATA-02 snapshot ids agree per player), `contractStatusAgeInvariant` (DATA-01's "never RFA/UFA plus a fabricated positive term" rule, restated as a check), `teamPopulationLineupInvariant` (wraps DATA-03's `rosterLegality`), `capPickReconciliation` and `retentionSlotGate` (wrap DATA-05's `applyCapDelta`/`retentionCheck`), `missingDataUncertaintyInvariant` (DATA-02's "never a surplus/interval with no market value behind it"), and `noFutureInformationLeakage` — genuinely new, since nothing in the repo previously checked that a historical or simulated `as_of` state carries no input dated after itself.
- **`app/lib/release-manifest.ts`** — the versioned manifest shape (`snapshotDate`, `modelVersion`, per-domain `DomainManifest`), and `buildDomainManifest`, which turns a domain's gate results into a `"live" | "degraded" | "down"` status: `down` on any failed gate, `degraded` when gates pass but a warning still applies (or when NO gates ran at all — an unchecked domain reads as unverified, never silently as healthy), `live` only when nothing is wrong. `failedDomains`/`manifestIsPublishable` let a caller diagnose or gate on exactly one domain without the others' state leaking in — the literal acceptance line.
- **`manifestCacheKey(base, snapshotDate, modelVersion)`** — wired into the two dynamically-keyed hot caches, `teamCacheKey` and `leagueTeamCacheKey` (`app/lib/team-cache.ts`), so a deployed `XNAV_MODEL_VERSION` bump now automatically produces a fresh cache key instead of continuing to serve a stale-shaped payload for up to the existing 6-hour TTL. Two existing tests that pinned the old bare key string (`cache:trade:teams:v1:cap:102.3`) were updated to build their expectation from the real function instead of a hardcoded literal.
- Evidence: 32 new/updated focused tests (`release-gates`, `release-manifest`, `team-cache-manifest`, plus two corrected cache-key assertions), full suite **2,310/2,310**, TypeScript clean, changed-file lint clean, production build passes (**30/30** static pages).
- Not yet done (remaining `DATA-06` scope): the manifest is not wired to any live ingest timestamp, coverage count, or warning for the eight named domains (roster/stats/contracts/valuation/picks/teamModel/fantasy/simulation) — that requires touching each domain's real data-assembly path, deliberately deferred rather than rushed without live data to verify against; the fully-static hot-path cache keys (`LEAGUE_ANALYTICS_CACHE_KEY`, `LEAGUE_TEAMS_PAYLOAD_CACHE_KEY`, `LEAGUE_PLAYERS_CACHE_KEY`) were **not** touched — they are the highest-traffic caches in the app (the M-Perf precompute's whole point) and rewiring their key shape without the ability to verify against live Redis/production traffic in this session was judged too risky; and no manifest is actually published anywhere (no admin panel or API route surfaces one yet) — the infrastructure exists and is tested, adoption does not yet.

**Progress — August 31, 2026 (increment 2 of `M`; ticket remains `[ ]`).** Wired four of the eight domains to live gates and published the manifest for the first time:

- **`app/lib/live-release-manifest.ts`** — `buildLiveReleaseManifest(snapshot)` takes an already-loaded roster snapshot (the same shape `getCachedRoster()` already returns — no new fetch) and reports on it. Only wired the domains a plain roster/navMap snapshot can actually verify without a specific trade proposal in flight: `capPickReconciliation`/`retentionSlotGate` need a real trade's before/after state and stay per-trade checks; `noFutureInformationLeakage` is inherently about a simulated/historical `as_of` state, which a live roster read is not. What IS wired: **roster** (`exactNameAliasInvariance` across every real player), **contracts** (`contractStatusAgeInvariant` + the new DATA-01 `freeAgentPoolConsistencyInvariant`), **valuation** (computes X-NAV twice — simulating two independent surfaces reading the same roster on the same day — and runs `crossSurfaceValueReconciliation` against both, the real determinism guarantee DATA-02 promises, exercised on live data instead of only a fixture; plus `missingDataUncertaintyInvariant`), and **teamModel** (`teamPopulationLineupInvariant` via `rosterLegality` across all 32 real teams). `stats`/`picks`/`fantasy`/`simulation` stay unwired — `buildDomainManifest`'s own rule reads an unwired domain as `degraded` with an explicit "no gates ran" warning, never fabricated as `live`.
- **Published for the first time**: `GET /api/admin/release-manifest` — admin-gated, reads the same cached roster every other route already reads, returns `{ ok, publishable, failedDomains, manifest }`.
- **Verified live against real production-shaped data** (this session's sandbox, `ADMIN_DISABLE_AUTH=1`): roster/contracts/valuation all read `live` against 60 real players with no alias collisions, no incomplete free-agent facts, and cross-surface reconciliation holding. **`teamModel` correctly read `down`** — this sandbox's fallback dataset has only 60 total players across 32 teams, nowhere near enough to legally staff any lineup, and the gate caught it rather than rubber-stamping. `publishable: false` and `failedDomains: ["teamModel"]` is exactly the acceptance line working: a real failed domain, diagnosed without marking roster/contracts/valuation down with it.
- Evidence: 12 new focused tests (`live-release-manifest`, `release-manifest-route`), full suite **2,394/2,394**, TypeScript and changed-file lint clean, production build passes (**30/30** static pages, including the new route).
- Remaining `DATA-06` scope: `stats`/`picks`/`fantasy`/`simulation` still unwired; the static hot-path cache keys remain untouched (same risk judgment as increment 1); and nothing consumes the manifest yet beyond the admin route returning it (no dashboard, no alerting, no gating of a deploy on `manifestIsPublishable`).
...
- [x] Freeze the market-calibration protocol and audit leakage/sample coverage (September 9, 2026): `scripts/backtest/nav-calibration-cohort.ts --market` separates 1,554 negotiated standard contracts with leakage-safe pre-signing NHL features from 1,609 CBA-constrained ELCs, 2,032 no-NHL-sample rows and 1,034 otherwise eligible unresolved identities. The market-only reference passes representation/sample gates (390/513/651 joined rows); the prior 280-ELC remedy is withdrawn because none can currently produce a pre-signing feature row. See `docs/analytics/NAV01_CONTRACT_CALIBRATION_PROTOCOL.md`; no public values or migration begin here.
- [x] Fit and validate the common F/D/G scale under that manifest; publish a reproducible pass/fail report (September 9, 2026). The committed, checksum-frozen shadow bridge failed its single holdout run: aggregate MAE gain was only 0.0334 cap-share points (95% player-cluster bootstrap -0.0701 to +0.1546), D regressed, and F/G retained excessive bias. Public NAV remains unchanged; see `docs/analytics/NAV01_SHADOW_CALIBRATION_REPORT.md`.
- [x] Complete the Phase 5 remediation diagnostic (September 10, 2026): the 651-row holdout is permanently development-only; F/G status and rolling-intercept corrections are unstable; a separate pre-signing D market bridge is promising in two rolling folds; and no full F/D/G candidate is ready for a new holdout. See `docs/analytics/NAV01_PHASE5_REMEDIATION.md`.
