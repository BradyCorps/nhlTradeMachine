...
Run states are deliberately limited to `PLANNED`, `COMPLETED`, `FAILED`, and `INVALIDATED`. None represents validation, approval, public availability, or promotion. `PLANNED` has no timestamps, result fingerprint, observations, or gate outcomes. `COMPLETED` requires ordered start/completion timestamps, a result-set fingerprint, every required observation and gate outcome, and no failure/invalidation reason. `FAILED` requires ordered timestamps and a failure reason and cannot carry a PASS outcome. `INVALIDATED` requires an invalidation reason and remains visible only as unusable historical evidence. Gate outcomes are `PASS`, `FAIL`, or `INCONCLUSIVE`.
