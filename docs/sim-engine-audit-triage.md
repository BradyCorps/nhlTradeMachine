...
### P1-6 · Player roles weakly affect scoring type 🔴 CONFIRMED
- **Root cause:** goal share is *only* the xG/pace ratio + a D special-case
...
### P1-9 · RFA state is contradictory and unauditable 🟡 PARTIAL (accurate, with nuance)
...
- **What the code does:** walk-aways *are* moved, not dropped — `walkPlayer` /
  `proceedToOfferSheets` push to `market` (`useOffseasonFlow.ts:153,202`); the
  league offseason tracks `walkAways` and re-homes them (`free-agency.ts:389,422,
  479,497,628`). **But** there is **no persistent, user-visible transaction ledger**
  and **no player-state conservation invariant**: pools are regenerated by seeded
  odds each offseason, so a walked player's RFA/UFA/"no comp"/signed-elsewhere state
  isn't reconciled or surfaced. The contradiction the audit saw is real *and*
  unauditable by construction.
- **Fix:** add a transaction ledger + the invariant `roster + retained rights +
  RFA pool + UFA pool + signed-elsewhere + retired = previous + drafted`, checked
  every offseason and exposed as a diagnostic. **M.**
