...
# POSTCODEX_AUDIT
- 2026-07-10: Fixed the Cup Run sim-stats carry-forward path so projected skater GP/G/A/PTS pace and TOI are stamped onto the rolled roster before `advanceSeason`, letting 0-game prospects become real year-to-year development inputs instead of reverting to 0 pts/0 TOI; sim skater seasons now expose projected TOI and the lifecycle passes `standings[].projectedSkaters` into rollover. Touched `app/api/simulate/route.ts`, `app/lib/cup-run.ts`, `app/armchair-gm/useCupRunLifecycle.ts`, `__tests__/cup-run.test.ts`, `__tests__/simulate-and-claude-routes.test.ts`.
- 2026-07-10: Began the modern forward archetype taxonomy by replacing EA-style `SNIPER`/`PLAYMAKER`/`SCORER`/`GRINDER` outputs with `HIGH_GRAVITY`, `LINE_RAISER`, `LINE_ESTABLISHER`, `LINE_FINISHER`, `IMPACT_PLAYER`, `DEFENSIVE`, `SPEED_BURST`, and `SPACE_OPENER` (`CLUTCH_PLAYER` reserved until a real clutch signal exists); shared the classifier with the players-page badge, threaded EDGE speed/burst into NAV classification, and updated regression tests. Touched `app/lib/xnav-engine.ts`, `app/lib/trade-types.ts`, `app/api/evaluate/route.ts`, `app/players/page.tsx`, `app/components/PercentileCard.tsx`, `__tests__/xnav.test.ts`.
