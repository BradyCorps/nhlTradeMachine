...
const SECTIONS: { id: string; title: string; paras: string[] }[] = [
  ...
  },
  {
    ...
    paras: [
      ...
      "Evidence, not just calibration: a year-over-year backtest (2015-2026, 5,722 consecutive-season pairs) shows a player's gravity force persists at r=0.68 — a genuinely stable trait, though a shade below STRAND traits (0.74-0.89). But it is a descriptor, not a forecast: force predicts next-season on-ice xGF% at r=0.36, below simply carrying a player's own prior on-ice result forward (r=0.43), and it correlates 0.66 with the same-season result it is partly built from. That is exactly why the field is shown as a labelled visualization and is kept out of valuation. The neutral-zone transition well — the model's one novel claim — has no historical tracking data and remains unvalidated. The full accounting lives in the Gravity model card.",
