...
const SECTIONS: { id: string; title: string; paras: string[] }[] = [
  {
    ...
    paras: [
      ...
      "The composite was validated end-to-end by summing roster NAV for every team across 252 team-seasons (2017–2024) and correlating against goal differential per game. Aggregate NAV explains R² = 0.45 of team performance variance, with per-season R² ranging from 0.42 to 0.57. Quintile separation is monotonic: the lowest-NAV quintile averages −0.60 goal differential per game while the highest averages +0.55 — the gap between a bottom-feeder and a contender. Ablation tests confirm the offensive engine as the strongest single predictor (R² = 0.51), with goalie evaluation independently validated at R² = 0.30. The age and contract-surplus components slightly reduce team-level prediction, which is expected — they price trade value (young players and cost-controlled deals), not current-season wins.",
      "At the individual level, NAV persists year to year at r = 0.80 across 4,453 consecutive-season pairs (2017–2024, minimum 20 GP) — more stable than the average STRAND trait (r = 0.74) and firmly in the 'very stable' tier. The on-ice components are the stickiest: offensive value r = 0.87, defensive value r = 0.82, age curve r = 0.89. Contract surplus is the least stable component (r = 0.72), and it is the one source of noise — on-ice True Market Value persists at r = 0.86 while full NAV drops to r = 0.80. The gap is legitimate: when a player signs a new deal, their trade value genuinely changes, and same-contract pairs persist at r = 0.79 versus r = 0.70 for contract-changed pairs. Top-quintile NAV players stay in the top quintile 69% of the time; 90% stay within one quintile. The composite is not noisier than its components — the blending is working.",
    ],
  },
  {
    id: "fmv",
  ...
  },
  {
    ...
    paras: [
      ...
      "F-NAV, D-NAV, and G-NAV on the Teams page are today a client-side positional SUM of each rostered player's already-computed X-NAV — forwards, defensemen, and goaltenders bucketed and added, with any below-replacement player clamped at zero so the three splits always reconcile back to the combined total. They are not yet independent position-specific models; that calibrated pipeline (frozen holdout seasons, goalie-to-skater equivalence testing, its own uncertainty and coverage) is tracked separately and will carry its own validation before it changes what these numbers mean. The population is always the signed active roster only — never draft picks, and never the unsigned reserve list — so a team's F/D/G split is traceable to the exact same player snapshots as its combined roster total.",
